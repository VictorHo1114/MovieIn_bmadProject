...
export function WatchlistSection() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = movieListStore.subscribe(() => {
      // 只更新本地狀態，過濾掉已移除的電影
      setWatchlist((prevList) => prevList.filter((item) => movieListStore.isInWatchlist(item.movie.id)))
    })
  ...
  if (watchlist.length === 0) {
    return (
      <div className="text-center py-20">
        ...
        <p className="text-sm text-gray-400">點擊電影卡片的「加入 Watchlist」按鈕來新增電影</p>
      </div>
    )
  }
  ...
  return (
    <div>
      <div className="mb-6">
        ...
        <p className="text-sm text-gray-500 mt-1">管理你想看的電影</p>
      </div>
