  ...
  return (
    <div>
      ...
      <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
        <SortableContext items={top10List.map((item) => item.id)} strategy={verticalListSortingStrategy}>
          {top10List.map((item, index) => (
            <SortableTop10Item key={item.id} item={item} index={index} onRemove={() => handleRemove(item)} />
          ))}
        </SortableContext>
      </DndContext>
    </div>
