...
"""
智能混合推薦服務 - Multi-Version Architecture

包含三個版本的推薦系統：
- Phase 3.5: Feature-First Hybrid (智能決策，Feature 或 Embedding)
- Phase 3.6: Embedding-First (語義優先，Feature 輔助驗證) ⭐ 最新
- Legacy: Simple Feature-Based (向後兼容)
...
# ============================================
# 智能混合配置（可調整實驗參數）
# Phase 3.5 Feature-First Hybrid 專用
# Phase 3.6 使用獨立配置檔案: phase36_config.py
# ============================================
HYBRID_CONFIG = {
    "decision_threshold": 20,        # Phase 3.5: 決策閾值（20 分以上觸發 Embedding）
    "feature_score_threshold": 15.0, # Feature 匹配分數閾值
    "min_candidates": 20,            # 最小候選數量
    "enable_logging": True,          # 是否顯示詳細日誌
    "enable_dual_engine": True,      # Phase 3.5: 啟用雙引擎並行模式
    "enable_quadrant_logic": True,   # Phase 3.5: 啟用四象限邏輯
    "quadrant_thresholds": {         # Phase 3.5: 四象限閾值
        "high_match": 0.5,           # 高 match_ratio 閾值
        "high_embedding": 0.45,      # 高 embedding similarity 閾值
    },
    "fusion_weights": {              # Phase 3 ⭐ 加權融合權重（舊版，被四象限取代）
        "feature_score": 0.4,        # Feature 匹配 40%
        "embedding_score": 0.3,      # 語義匹配 30%
        "match_ratio": 0.3,          # 符合比例 30%
    },
    "quadrant_weights": {            # Phase 3.5 ⭐ 四象限專屬權重
        "q1_perfect": {              # 高 Match + 高 Embedding (完美匹配)
            "feature": 0.35,
            "embedding": 0.45,       # 提高 Embedding 權重
            "match": 0.20,
        },
        "q2_feature_trust": {        # 高 Match + 低 Embedding (信任 Feature)
            "feature": 0.60,         # 信任用戶精確選擇
            "embedding": 0.10,
            "match": 0.30,
        },
        "q3_semantic_discovery": {   # 低 Match + 高 Embedding (語義發現)
            "feature": 0.20,
            "embedding": 0.60,       # Embedding 主導
            "match": 0.20,
        },
        "q4_filtered": {             # 低 Match + 低 Embedding (過濾)
            "feature": 0.50,
            "embedding": 0.00,
            "match": 0.50,
        },
